  ...
  return (
    <div id="servicos" className="bg-white py-24 sm:py-10 ">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-4 max-w-2xl sm:mt-20 lg:mt-6 lg:max-w-4xl">
          <dl className="grid max-w-2xl grid-cols-1 gap-x-8 gap-y-2 lg:max-w-none lg:grid-cols-2 lg:gap-y-16">
            {features.map((feature) => (
              <div
                key={feature.name}
                className="relative pl-16 transition-all duration-300 ease-in-out hover:bg-gray-50 rounded-lg p-4"
              >
                <dt className="text-base font-semibold text-gray-900">
                  <div className="p-1 absolute left-0 top-0 flex items-center justify-center rounded-md bg-indigo-600 transition-transform duration-300 ease-in-out hover:scale-110">
                    <feature.icon
                      aria-hidden="true"
                      className="h-10 w-10 text-white transition-colors duration-300 ease-in-out hover:text-indigo-300"
                    />
                  </div>
                  {feature.name}
                </dt>
