...
const plans: Plan[] = [
  ...
  },
  {
    ...
    features: [
      ...
      "Conexão com redes sociais (Facebook, Instagram etc).",
      "Adequado para desktop, tablet e celular",
      "Instalação de Google Analytics e Google Search Console",
      ...
      <div id="precos" className="bg-white px-6 py-12 sm:py-16 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`rounded-md bg-white p-6 shadow-sm ring-1 ${
            ...
            >
              ...
              {plan.price ? (
                <div>
                    <p className="mt-2 flex flex-row text-3xl font-bold text-gray-900">
                      R${plan.price.toFixed(2).replace(".", ",")}
                    </p>
                  <p>em até 4X no pix ou cartão</p>
                  <p className="mt-1 text-sm text-gray-500">
                    + {calculateParcel(plan.price)} ao mês para hospedagem e
                    manutenção
                  </p>
                </div>
              ) : (
